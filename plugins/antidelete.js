...
command(
	...
	async (message, match) => {
		...
		if (command === 'on') {
			...
			return await message.reply('_AntiDelete is now off for Group Chats and Direct Messages by default. Use "set gc" or "set dm" to enable._');
		}

		if (command === 'off gc') {
			await setAnti('gc', false);
			return await message.reply('_AntiDelete for Group Chats is now disabled._');
		}

		if (command === 'off dm') {
			await setAnti('dm', false);
			return await message.reply('_AntiDelete for Direct Messages is now disabled._');
		}
		...
		if (command === 'set gc') {
			...
			return await message.reply(`_AntiDelete for Group Chats ${newStatus ? 'enabled' : 'disabled'}_`);
		}
		...
		if (command === 'set dm') {
			...
			return await message.reply(`_AntiDelete for Direct Messages ${newStatus ? 'enabled' : 'disabled'}_`);
		}

		if (command === 'set all') {
			await setAnti('gc', true);
			await setAnti('dm', true);
			return await message.reply('_AntiDelete set for all chats_');
		}
