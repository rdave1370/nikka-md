...
class Message extends Base {
  ...
  async sendPoll(jid, question, options, pollType = 0) { const pollMessage = { pollCreationMessage: { name: question, options: options.map((option) => ({ optionName: option })), selectableOptionsCount: pollType, // 0 for single choice, 1 for multiple choice }, }; const message = generateWAMessageFromContent(jid, proto.Message.fromObject(pollMessage), { userJid: this.client.user.id, }); await this.client.relayMessage(jid, message.message, { messageId: message.key.id, }); return message; }
