...
class Message extends Base {
  ...
  async sendSlide(client, jid, title, message, footer, slides, quotedMessage = null) {
    const cards = await Promise.all(slides.map(async slide => {
      ...
      const buttonParamsJson = this.buildButtonParams(buttonType, textCommand, command, url);
      ...
      viewOnceMessage: {
        message: {
          ...
          interactiveMessage: proto.Message.InteractiveMessage.fromObject({
            ...
            contextInfo: {
              mentionedJid: quotedMessage ? [quotedMessage.sender] : [],
              forwardingScore: 999,
              isForwarded: true,
